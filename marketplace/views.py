...
#helper
from .cp2 import get_cart_count
       ...
       if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            ...
            try:
                ...
                try:
                    ...
                    check_cart.save()         
                    return JsonResponse({'status': 'success!', 'message': "Increased the quantity!", 'cart_counter': get_cart_count(request), 'item_quantity': check_cart.quantity})      
                except:
                    # create new cart
                    print("create cart")
                    ...
                    return JsonResponse({'status': 'success!', 'message': "added food to cart",'cart_counter': get_cart_count(request), 'item_quantity': check_cart.quantity})      
        ...
        try:
            try:
                ...
                if check_cart.quantity >1:
                    check_cart.quantity -=1
                    check_cart.save()
                    return JsonResponse({"status":'success!', 'message':'Decreased the quantity','cart_counter': get_cart_count(request) ,'item_quantity':check_cart.quantity})
                else:
                    check_cart.delete() 
                    quantity = 0
                    return JsonResponse({"status":'success!', 'message':'Deleted the cart item','cart_counter': get_cart_count(request), 'item_quantity':quantity})
